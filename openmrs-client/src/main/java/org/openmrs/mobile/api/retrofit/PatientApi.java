...
public class PatientApi extends RetrofitApi {
    ...
    /**
     * Update Patient
     * make it just like register short update then simple sync
     * @param patient
            ...
            call.enqueue(new Callback<Patient>() {
                ...
                public void onResponse(Call<Patient> call, Response<Patient> response) {
                    if (response.isSuccessful()) {
                        ...
                        ToastUtil.error("Patient[" + patient.getId() + "] cannot be updated due to server error" + response.message());
                        if (callbackListener != null) {
                            callbackListener.onErrorResponse();
                        }
                    }
                ...
                @Override
                public void onFailure(Call<Patient> call, Throwable t) {
                    ToastUtil.notify("Patient[" + patient.getId() + "] cannot be updated due to request error: " + t.toString());
                    if (callbackListener != null) {
                        callbackListener.onErrorResponse();
                    }
                }
